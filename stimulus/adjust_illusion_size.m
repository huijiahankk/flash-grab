...
alphaSectorMask = 255 * (rho > data.sectorRadiusIn & rho < sectorRadiusOut & phi > (- 90 - sectorArcAngle/2) & phi< (-90 + sectorArcAngle/2));
...
%             DrawFormattedText(wptr, str, 'center', 'center', blackcolor);
    ...
    while GetSecs - BlockOnset < testDuration  && respToBeMade
        ...
        if keyIsDown && ~prekeyIsDown   % prevent the same press was treated twice
            ...
            elseif keyCode(KbName('delete'))  % ||keyCode(KbName('1!'))
                wedgeTiltNow = wedgeTiltNow - 4 * wedgeTiltStep;

            elseif keyCode(KbName('pagedown')) % ||keyCode(KbName('3#'))
                wedgeTiltNow = wedgeTiltNow + 4 * wedgeTiltStep;
