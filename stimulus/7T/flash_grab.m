...
if 1
    ...
    debug = 'n';
%     expmark = 2;
    flashPresentFrame = 2.2;  % 2.2 means 3 frame
...
%     expmark = 2; % input('>>>Which experiment? (1 background tilt，flash vertical/2 flash/background vertical):  ');
    ...
    flashPresentFrame = 2.2; %input('>>>flash represent frames? (0.8/2.2):  ');
...
back.RotateTimesSetMat = [4 8 4 8 4 8 4 8];
...
for block = 1 : blockNumber
    ...
    for trial = 1:trialNumber
        ...
        back.RotateTimes = 0;
        back.RotateTimesSet = back.RotateTimesSetMat(trial);
        ...
        while GetSecs - BlockOnset < testDuration  && respToBeMade
            ...
            if back.CurrentAngle > back.ReverseAngle  + adjustSectorArcAngle + wedgeTiltNow
                back.SpinDirec = - 1;
                back.FlagSpinDirecA = back.SpinDirec;
                back.RotateTimes = back.RotateTimes + 1;
                
            elseif back.CurrentAngle < - back.ReverseAngle  - adjustSectorArcAngle  + wedgeTiltNow
                back.SpinDirec = 1;
            ...
            % present flash    back.FlagSpinDirecA ==  - 1  means  flash tilt right
            if data.flashTiltDirection(trial) == 1  && back.FlagSpinDirecA ==  - 1 &&  back.RotateTimes == back.RotateTimesSet
                ...
                flashPresentFlag = 1;
            elseif data.flashTiltDirection(trial) == 2  && back.FlagSpinDirecB ==  1  && back.RotateTimes == back.RotateTimesSet % flash tilt left
                responseFlag = responseFlag + 1;
...
%                     wedgeTiltNow = wedgeTiltNow + 4 * wedgeTiltStep;
            ...
            % flash present frame round to integer
            if flashPresentFlag
                WaitSecs((1/framerate) * flashPresentFrame);
                %             WaitSecs(1);
            end
