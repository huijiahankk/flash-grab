...
% The blindfield is in the left or right visual, you should revise from the line  293 to line 310
%from top vertical is 0 degree, right horizontal is 90 degree
...
if 0
    ...
    annulusWidth =  'CB'; % blindspot   artificialScotoma corticalBlindness
    particiType = 'CB';  % 'corticalBlindness'  'healthyParticipant'
    CBfieldDegree = 150; % the blindfield degree of corticalBlindness patient
    blindfield = 'r'; % l for left r for right
else
    ...
    prompt = {'subject''s name','barLocation(u for upper\l for lower\n for normal)',...
        ...
        'annulusPattern(blurredBoundary/sector)','annulusWidth(blindspot/artificialScotoma/CB)',...
        'particiType(CB/healthyParticipant)','CBfieldDegree(60)','blindfield(l/r)'};
    dlg_title = 'Set experiment parameters ';
    answer  = inputdlg(prompt,dlg_title);
    [sbjname,barLocation,condition,isEyelink,annulusPattern,annulusWidth,particiType,CBfieldDegree,blindfield] = answer{:};
    fprintf(['sbjname: %s\n','barLocation: %s\n','condition: %s\n','isEyelink: %d\n','annulusType: %s\n','annulusWidth: %s\n',...
        'particiType: %s\n','CBfieldDegree: %d\n','blindfield: %s\n'],...
        sbjname,barLocation,condition,isEyelink,annulusPattern,annulusWidth,particiType,CBfieldDegree,blindfield);
    isEyelink = str2num(isEyelink);
    CBfieldDegree = str2num(CBfieldDegree);
end
...
screenHeight = 33.5; % 26.8 cm  33.5
...
% else
%     if strcmp(condition,'invi2vi')
%         if strcmp(blindfield,'l')
%             barTiltStartUpper = 270;   %
%             barTiltStartLower = 270;    %
%         elseif strcmp(blindfield,'r')
%             barTiltStartUpper = 90;
%             barTiltStartLower = 90;
%         end
%     elseif strcmp(condition,'vi2invi')
%         if strcmp(blindfield,'l')
%             barTiltStartUpper = 300;
%             barTiltStartLower = 180;
%         elseif strcmp(blindfield,'r')
%             barTiltStartUpper = 75;
%             barTiltStartLower = 160;
%         end
%     end
% end


[barTiltStart,perceivedCondiStartDeg] = paraSetBarTiltEyelinkHealthyPatient(particiType, barLocation,annulusWidth,blindfield_from_horizontal_degree,CBfieldDegree,blindfield);

bar_only = repmat(barTiltStart,1,blockNumber);

barTiltStartHealthy = 270;
...
%                         Screen('DrawLines', wptr, allCoords, LineWithPix, blackcolor, [xCenter,yCenter]);
        ...
        if trialNumber == 1 ||  trial ~= trialNumber
            ...
            while respToBeMade
                ...
                if isEyelink % && strcmp(annulusWidth,'corticalBlindness') == 0
                    %                     Eyelink('Message','TRIALID %d',trial);
                    [x, y] = getEyelinkCoordinates();
                    if isnan(x) || isnan(y)
                        fprintf('Eye tracker lost track of eyes \n');
...
%                             Screen('DrawLines', wptr, allCoords, LineWithPix, blackcolor, [xCenter,yCenter]);
        ...
        elseif ~strcmp(condition,'normal') &&  trial == trialNumber
            ...
            barMovStep = 0.2;
            if barLocation == 'u'
                barTiltNow = perceivedCondiStartDeg + randn * 10;  % perceivedCondiStartDeg = 70;
            elseif  barLocation == 'l'
                barTiltNow = perceivedCondiStartDeg + randn * 10;  % perceivedCondiStartDeg = 70;
            end
            ...
            while respToBeMade
                ...
                else   % if strcmp(annulusWidth,'artificialScotoma')
