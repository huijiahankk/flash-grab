...
for trial = 1:trialNumber
    ...
    while GetSecs - scanOnset < 600 % stimlength(trial)+stimonset(trial)  % back.RotateTimes < testDuration %  % &&  respToBeMade
        frameCounter = frameCounter + 1;
        if frameCounter > size(rotation_sequence,2) % 60
            frameCounter = frameCounter-size(rotation_sequence,2);
        end
...
%             if stimtype(trial) == stimtype(1)  % rotate leftward  
                % illusion tilt left 
%                 if frameCounter == 60 %>60.5 && frameCounter<63.5
...
%             else    stimtype(1) == 2 % rotate rightward
%                 % illusion tilt right   
%                 if frameCounter>0.5 && frameCounter<1.5  % when 
%                     Screen('FillArc',wptr,redcolor,redSectorRectAdjust,157.5,sectorArcAngle);  %  wedgeTiltNow - 360/sectorNumber/2
%                     Screen('FillArc',wptr,bottomcolor,InnerSectorRectAdjust,157.5,sectorArcAngle); %wedgeTiltNow  - 360/sectorNumber/2
%                     
%                     if frameCounter == 1
