...
off_syn_out = [off_syn_out_u;off_sync_out_l] - bar_only_ave;
off_syn_in = [off_sync_in_u;off_syn_in_l] - bar_only_ave;
...
off_syn_out_ave = mean(mean(off_syn_out));
off_syn_in_ave = mean(mean(off_syn_in));
...
y = [0 off_syn_out_ave off_syn_in_ave flash_grab_out_ave perceived_location_out_ave flash_grab_in_ave perceived_location_in_ave];
...
set(gca, 'XTick', 1:7, 'XTickLabels', {'bar-only' 'off-sync-out' 'off-sync-in' 'grab-out' 'perc-out' 'grab-in' 'perc-in'},'fontsize',20,'FontWeight','bold');
...
if strcmp(artificialScotomaExp,'y')
    ...
    line([0 8], [(blindfield_from_horizontal_degree - bar_only_ave) (blindfield_from_horizontal_degree-bar_only_ave)],'LineWidth',1.5);
end
%----------------------------------------------------------------------
%  plot  each trial
%----------------------------------------------------------------------
...
eachtrial_off_sync_out = reshape(off_syn_out,1,numel(off_syn_out));
eachtrial_off_sync_in = reshape(off_syn_in,1,numel(off_syn_in));
...
for condition = 1: numel([off_syn_out_u;off_sync_out_l])
    plot(2,eachtrial_off_sync_out(condition),'r--o');
    plot(3,eachtrial_off_sync_in(condition),'b--o');
    plot(4,eachtrial_flash_grab_out(condition),'r--o');
    plot(5,eachtrial_perceived_location_out(condition),'r--o');
    plot(6,eachtrial_flash_grab_in(condition),'b--o');
    plot(7,eachtrial_perceived_location_in(condition),'b--o');
end
